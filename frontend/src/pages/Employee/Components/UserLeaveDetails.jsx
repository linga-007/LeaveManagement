          ...
          <thead className='bg-blue-600'>
            ...
            <thead className="divide-y divide-gray-200 bg-white">
              <tr className="bg-gray-50">
                {tableHead.map((val, index) => (
                  <th key={index} className="px-5 py-2  border border-gray-400 text-left text-xs text-gray-500 uppercase  text-balance break-words">
            ...
            <tbody>
              {currentData.map((val,index)=>  <tr key={index} className="border border-gray-400">
                  <td className='px-3 py-0 border border-gray-400 '><div className='flex justify-center items-center uppercase'>{val}</div></td>
